...
public class StructureFormRenderer extends FormRenderer<StructureForm>
{
    ...
    private void ensureLoaded()
    {
        ...
        Link fileLink = Link.create(file);
        File nbtFile = BBSMod.getProvider().getFile(fileLink);
        ...
        {
            String name = nbtFile.getName().toLowerCase();

            if (name.endsWith(".schem") || name.endsWith(".schematic"))
            {
                this.loadSchematic(nbtFile);
                return;
            }
        ...
        try (InputStream is = BBSMod.getProvider().getAsset(fileLink))
        {
            if (is == null)
            {
                return;
    ...
    private void loadSchematic(File file)
    {
        ...
        if (format != null)
        {
            try (ClipboardReader reader = format.getReader(new java.io.FileInputStream(file)))
            {
                ...
                this.loadFromClipboard(clipboard);
            }
            catch (Exception e)
            {
                e.printStackTrace();
    ...
    private void loadSchematic(InputStream is, String filename)
    {
        ClipboardFormat format = ClipboardFormats.findByAlias(filename.endsWith(".schem") ? "sponge" : "mcedit");
    ...
    private void loadFromClipboard(Clipboard clipboard)
    {
        ...
        this.size = new BlockPos(w, h, l);
        this.boundsMin = BlockPos.ORIGIN;
        this.boundsMax = new BlockPos(w, h, l);
        ...
        for (BlockVector3 pos : region)
        {
            ...
            if (!block.getBlockType().getMaterial().isAir())
            {
                ...
                {
                    /*
                     * Note: FabricAdapter.adapt(BaseBlock) requires a valid WorldEdit session or registry access
                     * which might not be fully initialized in this client-side renderer context.
                     *
                     * However, BaseBlock.getAsString() returns the standard block state string (e.g. "minecraft:stone[snowy=true]")
                     * which we can reliably parse using Minecraft's registry.
                     */
                    String blockStr = block.getAsString();
                    BlockState state = this.parseBlockStateString(blockStr);

                    if (state != null && !state.isAir())
                    {
                        int x = pos.getBlockX() - min.getBlockX();
                        int y = pos.getBlockY() - min.getBlockY();
                        int z = pos.getBlockZ() - min.getBlockZ();

                        this.blocks.add(new BlockEntry(state, new BlockPos(x, y, z)));
                    }
                }
                catch (Exception e)
                {
    ...
    private BlockState parseBlockStateString(String data)
    {
        try 
        {
            /* Format: minecraft:stone[snowy=true] */
            int bracketIndex = data.indexOf('[');
            String idStr = (bracketIndex > 0) ? data.substring(0, bracketIndex) : data;
            ...
            if (bracketIndex > 0 && bracketIndex < data.length() - 1)
            {
                String propsStr = data.substring(bracketIndex + 1, data.length() - 1);
                ...
                for (String prop : props)
                {
                    ...
                    if (kv.length == 2)
                    {
                        Property<?> property = block.getStateManager().getProperty(kv[0]);
                        if (property != null)
                        {
                             state = this.applyProperty(state, property, kv[1]);
                        }
                    }
                }
            }
